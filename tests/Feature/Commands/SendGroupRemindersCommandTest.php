...
class SendGroupRemindersCommandTest extends TestCase
{
    ...
    public function test_it_dispatches_7_day_reminder()
    {
        // given a group with an interview in 7 days
        ...
        Queue::assertPushed(SendGroupInterviewReminderJob::class, function ($job) use ($applicant) {
            return $job->applicantId === $applicant->id;
        });
        $this->assertNotNull($group->fresh()->last_reminded_at);
    }

    public function test_it_dispatches_3_day_reminder()
    {
        // given a group with an interview in 3 days
        $group = Group::factory()->create([
    ...
    {
        // given an inactive group with an interview in 7 days
        $group = Group::factory()->create([
            'date_time' => now()->addDays(7)->setHour(10)->setMinute(0),
            'is_active' => false,
        ]);
