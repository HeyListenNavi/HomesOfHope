...
class EvidencesRelationManager extends RelationManager
{
    ...
    protected static ?string $title = 'Evidencias de Visitas';

    protected static ?string $icon = 'heroicon-s-camera';
                ...
                Forms\Components\Section::make('Cargar Evidencia')
                    ...
                    ->schema([
                        ...
                        Forms\Components\Select::make('visit_id')
                            ->label('Visita')
                            ->relationship('visit', 'scheduled_at', fn ($query) => $query->where('family_profile_id', $this->getOwnerRecord()->id))
                            ->getOptionLabelFromRecordUsing(fn ($record) => $record->scheduled_at->locale('es')->translatedFormat('j \\d\\e F \\d\\e\\l Y'))
                            ->required()
                            ->searchable()
                            ->preload()
                            ->native(false),
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('visit.scheduled_at')
                    ->label('Visita')
                    ->date('d M Y')
                    ->sortable(),
